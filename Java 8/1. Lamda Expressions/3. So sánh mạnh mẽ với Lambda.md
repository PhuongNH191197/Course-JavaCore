...
Có một số hàm `Comparator.comparing` dành riêng cho một số kiểu dữ liệu nhất định
Ví dụ `comparingInt()` dành cho kiểu `int`, nhận vào một lambda
```java
public static <T> Comparator<T> comparingInt(ToIntFunction<? super T> keyExtractor)
```
Ví dụ sắp xếp một List số nguyên dùng `comparingInt()`
```java
