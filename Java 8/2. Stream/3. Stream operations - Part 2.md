...
Integer max = listOfIntegers
  .stream()                 // gọi stream() tạo Stream từ List
  .mapToInt(v -> v)         // gọi mapToInt(v -> v) trên Stream để lấy Integer Stream
  .max()                    // gọi max() để lấy giá trị lớn nhất
