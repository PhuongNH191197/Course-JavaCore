...
#### *Stream.reduce() with Accumulator:* `Optional<T> reduce(BinaryOperator<T> accumulator)`
...
#### *Stream.reduce() with Identity and Accumulator:* `T reduce(T identity, BinaryOperator<T> accumulator)`
...
#### *Stream.reduce() with Identity, Accumulator and Combiner: (chỉ sử dụng cho luồng song song)*
`U reduce(U identity,
