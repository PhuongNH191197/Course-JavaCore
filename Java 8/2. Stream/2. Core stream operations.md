...
List<Integer> evenNumbersList = list.parallelStream().filter(i -> i%2 == 0).collect(Collectors.toList());
...
```java
List<Integer> numbers = Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9);
Integer[] evenNumbersArr = list.parallelStream().filter(i -> i%2 == 0).toArray(Integer[]::new);
